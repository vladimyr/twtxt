...
function replyTo(e){e.preventDefault();var el=u("textarea#text");var text=document.getElementById("text");el.empty();text.value=u(e.target).data("reply");el.scroll();text.focus();var size=text.value.length;text.setSelectionRange(size,size);}
...
function deleteTwt(e){e.preventDefault();if(confirm("Are you sure you want to delete this twt? This cannot be undone!")){Twix.ajax({type:"DELETE",url:u("#twtForm").attr("action"),success:function(data){var hash=u(e.target).data("hash");u("#"+hash).remove();},});}}
